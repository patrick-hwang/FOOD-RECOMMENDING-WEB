...
const RestaurantDetail = ({ item, onBack, onShuffleAgain, onGetDirection, activeTags = [], onToggleTag, currentUser }) => {
    ...
    // Images từ item
    const imagesMenu = item?.imagesMenu?.length > 0 ? item.imagesMenu : [item?.imageUrl || "https://placehold.co/600x400"];
    const imagesView = item?.imagesViews?.length > 0 ? item.imagesViews : [item?.imageUrl || "https://placehold.co/600x400"];
    const currentImages = activeTab === 'menu' ? imagesMenu : (activeTab === 'view' ? imagesView : []);

    // Dữ liệu mẫu (Tags) - Dịch tags
    const defaultTags = ["#Localspecility", "#Open24/7", "#Takeaway", "#BanhMi", "#Vietnamese", "#<100.000VND", "#Take away"];
    const rawTags = item?.tags || defaultTags;
    const displayTags = rawTags.map(tagWithHash => {
        const rawTag = tagWithHash.replace(/^#/, '');
        return { label: t(rawTag), value: rawTag, withHash: tagWithHash };
    });

    // Dữ liệu mẫu (Views - Ảnh)
    const mockViews = imagesView;

    // Dữ liệu mẫu (Reviews)
    const mockReviews = [
        { id: 1, name: "Name", rating: 4, comment: "Highly recommend for tourists - who would like to try Phở with Saigon's tasting." },
        { id: 2, name: "Name", rating: 5, comment: "Highly recommend for tourists - who would like to try Phở with Saigon's tasting." },
        { id: 3, name: "Name", rating: 4, comment: "Highly recommend for tourists - who would like to try Phở with Saigon's tasting." },
    ];

    // Check bookmark status khi load
    useEffect(() => {
        const checkBookmarkStatus = async () => {
            ...
            try {
                ...
                setIsSaved(found);
            } catch (e) { console.error('Bookmark check error:', e); }
        };
        checkBookmarkStatus();
    }, [userId, item?.id]);
    ...
    }, [currentImages, activeTab]);

    // Reset slider khi chuyển tab
    useEffect(() => {
        setSliderIndex(0);
    }, [activeTab]);
    ...
    const handleBookmark = async () => {
        if (!userId) return alert(t('please_login') || "Please login first!");
                    ...
                    <>
                        ...
                        <div className="rd-info-container">
                            <div className="rd-name-row">
                                ...
                                <div className="rd-bookmark-icon" onClick={handleBookmark} style={{ cursor: 'pointer' }}>
                            ...
                            <div className="rd-address-row">
                                ...
                                <span className="rd-text-info">{t('address') || 'Address'}: {item?.address || "197A Nguyễn Trãi, District 1, HCMC"}</span>
                            </div>

                            <div className="rd-address-row">
                                <div className="rd-icon-col"><DetailIcons.Clock /></div>
                                <span className="rd-text-info">{t('open') || 'Open'}: {item?.openTime || "07:00 AM - 11:00 PM"}</span>
                            </div>
                        </div>
                        ...
                        <div className="rd-tags-wrapper">
                            <div className="rd-tags-green-header">
                                {t('tag_information') || 'Tag information & Filters'}
                            </div>
                            <div className="rd-tags-content">
                                <p className="rd-tags-note">{t('tap_tags') || 'Tap tags to select'}</p>
                                    ...
                                    {displayTags.map((tag, i) => {
                                        ...
                                        return (
                                            <button
                                                ...
                                                onClick={() => onToggleTag && onToggleTag(tag.withHash)}
                                            >
                                                #{tag.label}
                                            </button>
                                        );
        ...
        <div className="rd-screen">
            ...
            <div className="rd-fixed-top">
                ...
                <div className="rd-tabs-row">
                    ...
                    >
                        {t('menu') || 'Menu'} ({imagesMenu.length})
                    </button>
                    <button
                        className={`rd-tab ${activeTab === 'view' ? 'active' : ''}`}
                        onClick={() => setActiveTab('view')}
                    >
                        {t('view') || 'View'} ({mockViews.length})
                    </button>
                    <button
                        className={`rd-tab ${activeTab === 'review' ? 'active' : ''}`}
                        onClick={() => setActiveTab('review')}
                    >
                        {t('review') || 'Review'} ({mockReviews.length})
                    </button>
                </div>
            </div>
